...
import zipfile
import json
import time  # Tambahan untuk timing
...
def deteksi():
    # Mulai timing untuk keseluruhan proses
    ...
    # Variabel untuk menyimpan waktu deteksi
    individual_processing_times = []
    total_inference_time = 0
    total_preprocessing_time = 0
    total_postprocessing_time = 0

    print(f"🚀 Memulai deteksi untuk {len(uploaded_files)} gambar...")

    for i, file in enumerate(uploaded_files, 1):
        if file:
            filename = secure_filename(file.filename)
            print(f"📸 Memproses gambar {i}/{len(uploaded_files)}: {filename}")
            
            # Timing untuk satu gambar
            ...
            for j in range(len(prediction['boxes'])):
                ...
                if score > CONFIDENCE_THRESHOLD:
                    ...
                    label_idx = prediction['labels'][j].item()
            ...
            # Hitung waktu total untuk gambar ini
            end_time_image = time.time()
            image_processing_time = end_time_image - start_time_image
            individual_processing_times.append(image_processing_time)
            
            # Log waktu untuk gambar ini
            print(f"   ⏱️  Waktu pemrosesan: {image_processing_time:.3f}s (Preprocessing: {preprocessing_time:.3f}s, Inference: {inference_time:.3f}s, Postprocessing: {postprocessing_time:.3f}s)")
            
            # Tambahkan informasi timing ke summary
            summary_content += f"Waktu pemrosesan: {image_processing_time:.3f} detik\n"
            summary_content += f"  - Preprocessing: {preprocessing_time:.3f}s\n"
            summary_content += f"  - Inference: {inference_time:.3f}s\n"
            summary_content += f"  - Postprocessing: {postprocessing_time:.3f}s\n"
            summary_content += "\n"
    ...
    avg_processing_time = sum(individual_processing_times) / len(individual_processing_times) if individual_processing_times else 0
    min_processing_time = min(individual_processing_times) if individual_processing_times else 0
    max_processing_time = max(individual_processing_times) if individual_processing_times else 0
    
    # Tambahkan ringkasan timing ke summary
    summary_content += "=== RINGKASAN WAKTU PEMROSESAN ===\n"
    summary_content += f"Total waktu pemrosesan: {total_processing_time:.3f} detik\n"
    summary_content += f"Jumlah gambar: {len(uploaded_files)}\n"
    summary_content += f"Rata-rata waktu per gambar: {avg_processing_time:.3f} detik\n"
    summary_content += f"Waktu tercepat: {min_processing_time:.3f} detik\n"
    summary_content += f"Waktu terlama: {max_processing_time:.3f} detik\n"
    summary_content += f"Total waktu preprocessing: {total_preprocessing_time:.3f} detik\n"
    summary_content += f"Total waktu inference: {total_inference_time:.3f} detik\n"
    summary_content += f"Total waktu postprocessing: {total_postprocessing_time:.3f} detik\n"
    summary_content += f"Kecepatan pemrosesan: {len(uploaded_files)/total_processing_time:.2f} gambar/detik\n"
    
    # Log ke console
    print(f"✅ Selesai! Total waktu: {total_processing_time:.3f}s untuk {len(uploaded_files)} gambar")
    print(f"📊 Rata-rata: {avg_processing_time:.3f}s per gambar")
    print(f"🚀 Kecepatan: {len(uploaded_files)/total_processing_time:.2f} gambar/detik")
    ...
    # Hitung dan simpan statistik deteksi dengan informasi timing
    stats = calculate_detection_stats(all_results)
    
    # Tambahkan statistik timing ke stats
    stats.update({
        ...
        'processing_speed': len(uploaded_files)/total_processing_time if total_processing_time > 0 else 0
    })
